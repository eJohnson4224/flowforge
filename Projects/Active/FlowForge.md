...
- Digitakt transfer (Elektroid) exists as **scaffolding**:
  - UI: “Transfer to Digitakt” button in `RandomSampleView` (currently shows a placeholder alert)
  - Service: `ElektroidCLI` wrapper (CLI path detection, device listing, transfer command), not yet fully wired/validated end-to-end
...
5. **Hardware integration (optional)**
   ...
   - **Digitakt transfer (Elektroid) – current status**
     - ✅ UI button exists in Random Sample Explorer (`RandomSampleView`)
     - ✅ `ElektroidCLI` wrapper exists (find CLI, `ld` device list, `df` storage check, `elektron:sample:ul` transfer)
     - ⚠️ Not currently hooked up end-to-end from the button (button still shows placeholder alert)
     - ⚠️ Not yet validated with a real Digitakt connection from the sandboxed app
   - **Digitakt transfer (Elektroid) – next debugging steps**
     1. **Verify Elektroid works outside the app** (Terminal)
        - `which elektroid-cli`
        - `elektroid-cli -a ld` (Digitakt should appear)
        - (Optional) `elektroid-cli -a df <device>:/` (storage)
        - (Optional) `elektroid-cli -a elektron:sample:ul <file.wav> <device>:/incoming`
     2. **Wire the UI button to the real implementation**
        - Replace placeholder `transferToDigitakt(...)` in `RandomSampleView` with calls into `ElektroidCLI`
        - Capture **stdout + stderr** and surface failures in UI (not only console)
     3. **Confirm sandbox + file access constraints**
        - If `elektroid-cli` errors with “permission denied”/cannot open file: stage the sample into the app container (temp/Application Support) before transfer
        - Ensure security-scoped folder URLs are handled correctly for any files coming from user-selected locations
     4. **Improve device targeting**
        - If `ld` returns multiple Digitakt endpoints, add a simple device picker or deterministic selection rule
     5. **Make the CLI path robust**
        - Add Settings field to set/override the `elektroid-cli` path when not discoverable via common paths/PATH
     6. **Define “working” acceptance criteria**
        - Click “Transfer to Digitakt” → device detected → file uploaded to `/incoming` → success confirmation with any warnings shown
