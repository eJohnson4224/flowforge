...
- UI state routing: `FlowForge/ContentView.swift`
- Global state + persistence: `FlowForge/Models/AppState.swift`
- Folder selection/scanning/moves: `FlowForge/Services/FolderManager.swift`
- Sketches samples scan + Ableton linking:
  - `FlowForge/Services/SampleScanner.swift`
  - `FlowForge/Services/AbletonParser.swift`
- Audio preview engine: `FlowForge/Services/AudioPreviewManager.swift`
- Sample UX:
  - `FlowForge/Views/SamplesListView.swift`
  - `FlowForge/Views/SampleDetailView.swift`
  - `FlowForge/Views/RandomSampleView.swift`
...
Historical phase planning docs have been archived under `docs/archive/`.
Phase completion tracking lives in `docs/archive/done.md`.
Proof Test Sweep is defined in `docs/active/test-sweep.md` and gates phase transitions.
...
- Run `docs/active/test-sweep.md` after significant changes that warrant a commit
- Passing the sweep is required to exit Phase S and enter Phase R
- Record the pass in `docs/archive/done.md`

### Transfer edge-case test plan (commit gate detail)

Run these in order after any significant transfer changes (mirrors `docs/active/test-sweep.md`):
...
- `docs-second-brain/README.md` for execution workflow
- `docs-second-brain/now.md` for active phase priorities
- `docs-second-brain/file-management-workflow.md` for move/scan reliability
- `docs-second-brain/context-digitakt-object-model.md` for Digitakt memory/object model reasoning
